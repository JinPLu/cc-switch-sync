...
2. 把 `servers.conf.example` 复制为 `servers.conf`，填入服务器信息
...
| 脚本 | 用途 |
|------|------|
| `1. SSH Connect.bat` | 连接服务器。首次使用时选「+ Add new server」，脚本会自动初始化远程环境（安装依赖、同步配置），然后直接连入 |
| `2. Sync Config.bat` | 在 CC Switch 里切换了 Provider 后，把新配置推送到服务器 |
